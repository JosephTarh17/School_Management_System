...
import { ENUMS, ApiError, asDate, asNumber, asUuid, asyncRoute, sendData } from '../lib/api.js'
...
const select = 'record_id,student_id,assessment_id,score,grade,evaluation_date,published,updated_at,student(student_id,full_name),assessment(assessment_id,course_id,title,assessment_type,max_score,weight,course(course_id,course_code,course_name))'
...
async function recalculateFinalGrade(studentId, courseId) {
  const [{ data: assessments, error: assessmentError }, { data: records, error: recordError }] = await Promise.all([
    supabase.from('assessment').select('assessment_id,max_score,weight').eq('course_id', courseId),
...
async function assertStudentInCourse(studentId, courseId) {
  ...
  if (error) throw error
  if (!data) throw new ApiError(400, 'Student is not actively enrolled in this course')
}
...
router.get('/final-grades', asyncRoute(async (req, res) => {
  let query = supabase.from('final_grade').select('final_grade_id,student_id,course_id,computed_score,letter_grade,gpa,course(course_id,course_code,course_name)').order('course_id')
  if (req.user.role === 'student') {
    const studentId = await studentIdForUser(req.user.user_id)
    if (!studentId) return sendData(res, [])
...
router.post('/', requireRole('teacher', 'administrator'), asyncRoute(async (req, res) => {
  ...
  await assertTeacherOwnsCourse(assessment.course_id, req)
...
router.patch('/:recordId', requireRole('teacher', 'administrator'), asyncRoute(async (req, res) => {
  ...
  const { data: current, error: currentError } = await supabase.from('academic_record').select('record_id,student_id,assessment_id,assessment(course_id,max_score)').eq('record_id', recordId).maybeSingle()
  if (currentError) throw currentError
  if (!current) throw new ApiError(404, 'Academic record not found')
  await assertTeacherOwnsCourse(current.assessment.course_id, req)
  const updates = {}
  ...
  if (error) throw error
  await recalculateFinalGrade(current.student_id, current.assessment.course_id)
  return sendData(res, data)
}))
