  ...
  <div class="space-y-6 max-w-4xl">
    ...
    <div class="bg-white rounded-xl border border-border-subtle p-6 shadow-xs space-y-6" v-if="currentUser">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-geist">
        ...
        <div>
          <label class="block font-bold text-slate-700 mb-1">Email Address</label>
                      <input v-model="email" type="email" :disabled="currentUser.role === 'guardian'" class="w-full px-3 py-2 border border-slate-200 rounded-eight bg-slate-50 font-sans text-sm disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500" />
            <p v-if="currentUser.role === 'guardian'" class="mt-1 text-[11px] text-slate-500">Guardian email changes require administrator review. Use Guardian Engagement.</p>
      ...
      <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
        <RouterLink v-if="currentUser.role === 'guardian'" to="/guardian-engagement" class="btn-secondary px-4 py-2 text-xs font-semibold">Open Guardian Engagement</RouterLink>
        <button v-else @click="saveProfile" :disabled="saving" class="btn-primary px-4 py-2 text-white text-xs font-semibold font-geist disabled:opacity-60">
