      ...
      <div v-else>
        <div class="space-y-3 md:hidden">
          <article v-for="record in logs" :key="record.participation_id" class="rounded-lg border border-slate-200 bg-slate-50/60 p-4">
            ...
            <div class="mt-4 grid grid-cols-2 gap-2"><button type="button" @click="openEdit(record)" class="rounded border border-blue-200 px-2.5 py-2 font-semibold text-blue-700 hover:bg-blue-50">Edit</button><button type="button" :disabled="deletingId === record.participation_id" @click="deleteLog(record)" class="rounded border border-rose-200 px-2.5 py-2 font-semibold text-rose-700 hover:bg-rose-50 disabled:opacity-50">{{ deletingId === record.participation_id ? 'Deleting…' : 'Delete' }}</button></div>
          </article>
        </div>
