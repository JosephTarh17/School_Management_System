<template>
  <div class="space-y-6">
    ...
    <div class="rounded-xl border border-border-subtle bg-white p-6 shadow-xs">
      ...
      <div v-else>
        <div class="space-y-3 md:hidden">
          <article v-for="record in logs" :key="record.participation_id" class="rounded-lg border border-slate-200 bg-slate-50/60 p-4">
            <div class="flex items-start justify-between gap-3"><div class="min-w-0"><p class="break-all text-[11px] font-bold text-slate-900">{{ record.participation_id }}</p><p class="mt-1 truncate text-sm font-semibold text-slate-900">{{ record.student?.full_name || record.student_id }}</p></div><Badge class="shrink-0" :type="badgeType(record.rating)" :text="record.rating" /></div>
            <dl class="mt-4 space-y-2 text-xs"><div><dt class="text-slate-500">Course</dt><dd class="mt-0.5 font-medium text-slate-800">{{ record.class_session?.course?.course_name || record.session_id }}</dd></div><div><dt class="text-slate-500">Notes</dt><dd class="mt-0.5 break-words text-slate-600">{{ record.notes || 'No note recorded' }}</dd></div><div><dt class="text-slate-500">Recorded</dt><dd class="mt-0.5 text-slate-600">{{ formatDate(record.recorded_at) }}</dd></div></dl>
            <div class="mt-4 grid grid-cols-2 gap-2"><button type="button" @click="openEdit(record)" class="btn-secondary px-2.5 py-2 text-xs font-semibold">Edit</button><button type="button" :disabled="deletingId === record.participation_id" @click="deleteLog(record)" class="btn-danger px-2.5 py-2 text-xs font-semibold">{{ deletingId === record.participation_id ? 'Deleting…' : 'Delete' }}</button></div>
          </article>
        </div>
        <div class="hidden overflow-x-auto md:block"><table class="w-full min-w-[900px] text-left text-xs font-geist"><thead><tr class="border-b border-slate-200 bg-slate-50 text-slate-500"><th class="px-4 py-3 font-semibold">Entry ID</th><th class="px-4 py-3 font-semibold">Student</th><th class="px-4 py-3 font-semibold">Course</th><th class="px-4 py-3 font-semibold">Rating</th><th class="px-4 py-3 font-semibold">Notes</th><th class="px-4 py-3 font-semibold">Recorded</th><th class="px-4 py-3 font-semibold">Actions</th></tr></thead><tbody class="divide-y divide-slate-100 text-slate-700"><tr v-for="record in logs" :key="record.participation_id" class="hover:bg-slate-50/80"><td class="px-4 py-3.5 font-bold text-slate-900">{{ record.participation_id }}</td><td class="px-4 py-3.5 font-medium text-slate-900">{{ record.student?.full_name || record.student_id }}</td><td class="px-4 py-3.5">{{ record.class_session?.course?.course_name || record.session_id }}</td><td class="px-4 py-3.5"><Badge :type="badgeType(record.rating)" :text="record.rating" /></td><td class="max-w-xs px-4 py-3.5 text-slate-600">{{ record.notes || 'No note recorded' }}</td><td class="px-4 py-3.5 text-slate-500">{{ formatDate(record.recorded_at) }}</td><td class="px-4 py-3.5"><div class="flex gap-2"><button type="button" @click="openEdit(record)" class="btn-secondary px-2.5 py-1.5 text-xs font-semibold">Edit</button><button type="button" :disabled="deletingId === record.participation_id" @click="deleteLog(record)" class="btn-danger px-2.5 py-1.5 text-xs font-semibold">{{ deletingId === record.participation_id ? 'Deleting…' : 'Delete' }}</button></div></td></tr></tbody></table></div>
      </div>
    </div>
    ...
    <div v-if="editingId" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4" role="dialog" aria-modal="true" aria-labelledby="edit-participation-title">
      <form class="max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-4 shadow-xl sm:p-6" @submit.prevent="saveEdit"><div class="flex items-start justify-between"><div><h2 id="edit-participation-title" class="text-lg font-bold text-slate-900">Edit participation record</h2><p class="mt-1 text-xs text-slate-500">{{ editingRecord?.participation_id }}</p></div><button type="button" @click="closeEdit" class="btn-ghost p-1 text-slate-400 hover:text-slate-700" aria-label="Close edit form">✕</button></div>
