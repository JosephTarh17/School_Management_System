<template>
  <section class="space-y-6">
    <header class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      ...
      <button @click="load" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white">Refresh</button>
    </header>
    ...
    <form v-if="canManage" @submit.prevent="saveIncident" class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <button :disabled="saving" class="mt-4 rounded-lg bg-slate-950 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{{ saving ? 'Saving…' : 'Save incident' }}</button>
    </form>
