...
async function loadCalendar() { loading.value = true; errorMessage.value = ''; const result = await fetchCalendar(authStore.token.value, filters); if (!result.ok) errorMessage.value = result.error || 'Unable to load the calendar.'; else events.value = result.data?.events || []; loading.value = false }
