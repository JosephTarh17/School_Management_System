  ...
  <section class="space-y-6">
    <header class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <p class="mt-1 max-w-3xl text-sm text-slate-500">Enable or disable user accounts without deleting academic, attendance, financial, or audit history.</p>
      </div>
      ...
      <div v-else class="mt-5 grid grid-cols-1 gap-3 xl:grid-cols-2">
        <article v-for="user in filteredUsers" :key="user.user_id" class="rounded-lg border border-slate-200 p-4">
          <div class="flex items-start justify-between gap-3">
            ...
            <span :class="user.disabled_at ? 'bg-rose-50 text-rose-700' : 'bg-emerald-50 text-emerald-700'" class="shrink-0 rounded-full px-2.5 py-1 text-[11px] font-semibold">
          ...
          <div class="mt-4 flex flex-wrap items-center gap-2">
            <button v-if="user.disabled_at" type="button" :disabled="busyUserId === user.user_id" class="btn-primary px-3 py-2 text-xs font-semibold disabled:opacity-50" @click="enableAccount(user)">
              {{ busyUserId === user.user_id ? 'Enabling…' : 'Enable account' }}
            </button>
            <button v-else type="button" :disabled="!canDisable(user) || busyUserId === user.user_id" class="btn-danger px-3 py-2 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-50" @click="openDisable(user)">
              Disable account
            </button>
            <span v-if="!canDisable(user) && !user.disabled_at" class="text-xs text-slate-500">{{ user.user_id === currentUserId ? 'Your own account cannot be disabled.' : 'Keep one active administrator available.' }}</span>
          </div>
        </article>
      </div>
    </div>
...
import { fetchUsers, updateUserStatus } from '../api.js'
...
function canDisable(user) {
  if (user.user_id === currentUserId.value) return false
  if (user.role === 'administrator' && activeAdministratorCount.value <= 1) return false
  return true
}

function openDisable(user) {
...
async function saveStatus(user, body, message) {
  ...
  if (!result.ok) {
    errorMessage.value = result.error || 'Unable to update account status.'
  } else {
