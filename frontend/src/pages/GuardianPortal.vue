  ...
  <section class="space-y-6">
    ...
    <template v-else-if="summary">
      ...
      <div class="grid grid-cols-1 gap-5 xl:grid-cols-2">
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h2 class="mb-4 font-bold text-slate-900">Financial status</h2><div v-if="!summary.financial_records.length" class="text-sm text-slate-500">No financial records available.</div><div v-for="invoice in summary.financial_records" :key="invoice.invoice_id" class="flex items-center justify-between border-b py-3 last:border-0"><div><p class="font-semibold text-slate-900">Invoice {{ invoice.invoice_id.slice(0, 8) }}</p><p class="text-xs text-slate-500">Due {{ invoice.due_date || 'Not specified' }}</p></div><div class="text-right"><p class="font-bold text-slate-900">{{ formatXaf(invoice.amount_paid) }} / {{ formatXaf(invoice.amount_due) }}</p><p class="text-xs text-slate-500">Balance {{ formatXaf(invoice.balance_due ?? Math.max(Number(invoice.amount_due || 0) - Number(invoice.amount_paid || 0), 0)) }} · {{ invoice.payment_status }}</p><p v-if="invoice.last_payment_at" class="text-xs text-slate-400">Last payment {{ new Date(invoice.last_payment_at).toLocaleDateString() }}</p></div></div></div>
      </div>
    </template>
  </section>
</template>
