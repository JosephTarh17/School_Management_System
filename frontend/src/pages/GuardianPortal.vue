        ...
        <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div v-for="invoice in summary.financial_records" :key="invoice.invoice_id" class="border-b py-4 last:border-0">
            <div class="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div class="text-left sm:text-right"><p class="break-words font-bold text-slate-900">{{ formatXaf(invoice.amount_paid) }} / {{ formatXaf(invoice.amount_due) }}</p><p class="text-xs text-slate-500">Balance {{ formatXaf(invoice.balance_due ?? Math.max(Number(invoice.amount_due || 0) - Number(invoice.amount_paid || 0), 0)) }} · {{ invoice.payment_status }}</p></div>
            </div>
            ...
            <div v-if="invoice.installments?.length" class="mt-4 rounded-lg bg-slate-50 p-3">
              <div class="mb-2 flex flex-col items-start gap-1 sm:flex-row sm:items-center sm:justify-between"><h3 class="text-xs font-bold uppercase tracking-wide text-slate-600">Guardian-paid installment schedule</h3><span class="text-[11px] text-slate-500">{{ invoice.installments.length }} installment(s)</span></div>
