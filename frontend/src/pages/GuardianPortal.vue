        ...
        <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div v-for="invoice in summary.financial_records" :key="invoice.invoice_id" class="border-b py-4 last:border-0">
            <div class="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div class="flex w-full flex-col gap-2 text-left sm:w-auto sm:items-end sm:text-right"><div><p class="break-words font-bold text-slate-900">{{ formatXaf(invoice.amount_paid) }} / {{ formatXaf(invoice.amount_due) }}</p><p class="text-xs text-slate-500">Balance {{ formatXaf(invoiceBalance(invoice)) }} · {{ invoice.payment_status }}</p></div><button v-if="!invoice.installments?.length && invoiceBalance(invoice) > 0" type="button" :disabled="isPaymentLoading(invoice.invoice_id)" @click="startCinetPayPayment(invoice)" class="rounded-lg bg-indigo-600 px-3 py-2 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">{{ isPaymentLoading(invoice.invoice_id) ? 'Opening payment…' : `Pay ${formatXaf(invoiceBalance(invoice))} with CinetPay` }}</button></div>
            </div>
            ...
            <div v-if="invoice.installments?.length" class="mt-4 rounded-lg bg-slate-50 p-3">
              ...
              <div v-for="installment in invoice.installments" :key="installment.installment_id" class="flex flex-col gap-2 border-t border-slate-200 py-2 text-xs sm:flex-row sm:items-center sm:justify-between">
                ...
                <button v-if="Number(installment.balance_due || 0) > 0" type="button" :disabled="isPaymentLoading(installment.installment_id)" @click="startCinetPayPayment(invoice, installment)" class="rounded-lg bg-indigo-600 px-3 py-2 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">{{ isPaymentLoading(installment.installment_id) ? 'Opening payment…' : `Pay ${formatXaf(installment.balance_due)} with CinetPay` }}</button>
              </div>
            </div>
