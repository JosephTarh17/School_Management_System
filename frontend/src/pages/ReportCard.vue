<template>
  <section class="space-y-6">
    ...
    <div v-else class="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center text-sm text-slate-500">No administrator-published report card is available for this term.</div>
  </section>
</template>
...
async function load() {
  ...
  if (!studentId) return
  const result = await fetchReportCard(authStore.token.value, studentId, term.value)
