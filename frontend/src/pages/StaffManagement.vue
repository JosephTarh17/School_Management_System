      ...
      <form class="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm" @submit.prevent="submitStaff">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <label class="text-xs font-semibold text-slate-700">Email
            <input v-model.trim="form.email" :required="form.staff_type === 'teaching'" type="email" maxlength="320" :placeholder="form.staff_type === 'teaching' ? 'employee@example.com' : 'Optional email'" class="mt-1.5 block w-full rounded-lg border px-3 py-2 text-sm font-normal focus:border-indigo-500 focus:outline-none" />
          </label>
        ...
        <button type="submit" :disabled="saving" class="btn-primary w-full px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{{ saving ? 'Saving…' : editingId ? 'Save staff changes' : 'Add staff member' }}</button>
      </form>
      ...
      <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div v-else class="space-y-3">
          <article v-for="member in staff" :key="member.staff_id" class="rounded-lg border border-slate-200 p-4">
            ...
            <div class="mt-4 flex flex-wrap gap-2"><button type="button" class="btn-secondary px-3 py-2 text-xs font-semibold" @click="startEdit(member)">Edit</button><button type="button" class="btn-danger px-3 py-2 text-xs font-semibold" @click="removeStaff(member)">Delete</button></div>
          </article>
        </div>
      </div>
    </div>
