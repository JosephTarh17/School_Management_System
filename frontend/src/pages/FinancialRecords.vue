    ...
    <div class="rounded-xl border border-border-subtle bg-white p-4 shadow-xs sm:p-6">
      ...
      <div v-else>
        <div class="space-y-3 md:hidden">
          <article v-for="record in transactions" :key="record.invoice_id" class="rounded-lg border border-slate-200 bg-slate-50/60 p-4">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0"><p class="break-all text-xs font-bold text-slate-900">{{ record.invoice_id }}</p><p class="mt-1 truncate text-sm font-semibold text-slate-900">{{ record.student?.full_name || record.student_id }}</p></div>
              <Badge class="shrink-0" :type="record.payment_status === 'Paid' ? 'present' : 'warning'" :text="record.payment_status" />
            </div>
            <dl class="mt-4 grid grid-cols-2 gap-x-3 gap-y-3 text-xs">
              <div><dt class="text-slate-500">Amount due</dt><dd class="mt-0.5 font-semibold text-slate-900">{{ formatCurrency(record.amount_due) }}</dd></div>
              <div><dt class="text-slate-500">Paid</dt><dd class="mt-0.5 font-semibold text-slate-900">{{ formatCurrency(record.amount_paid) }}</dd></div>
              <div class="col-span-2"><dt class="text-slate-500">Balance</dt><dd class="mt-0.5 font-semibold text-slate-900">{{ formatCurrency(record.balance_due ?? Math.max(Number(record.amount_due || 0) - Number(record.amount_paid || 0), 0)) }}</dd></div>
            </dl>
            <div class="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3">
              <button type="button" @click="openPayment(record)" class="btn-primary px-2.5 py-2 text-xs font-semibold">Record payment</button>
              <button type="button" @click="openEdit(record)" class="btn-secondary px-2.5 py-2 text-xs font-semibold">Edit</button>
              <button type="button" :disabled="deletingId === record.invoice_id" @click="deleteRecord(record)" class="btn-danger px-2.5 py-2 text-xs font-semibold">{{ deletingId === record.invoice_id ? 'Deleting…' : 'Delete' }}</button>
            </div>
          </article>
        </div>
        <div class="hidden overflow-x-auto md:block"><table class="w-full min-w-[980px] text-left text-xs font-geist"><thead><tr class="border-b border-slate-200 bg-slate-50 text-slate-500"><th class="px-4 py-3 font-semibold">Invoice</th><th class="px-4 py-3 font-semibold">Student</th><th class="px-4 py-3 font-semibold">Amount due (XAF)</th><th class="px-4 py-3 font-semibold">Paid (XAF)</th><th class="px-4 py-3 font-semibold">Balance (XAF)</th><th class="px-4 py-3 font-semibold">Status</th><th class="px-4 py-3 font-semibold">Actions</th></tr></thead><tbody class="divide-y divide-slate-100 text-slate-700"><tr v-for="record in transactions" :key="record.invoice_id" class="hover:bg-slate-50/80"><td class="px-4 py-3.5 font-bold text-slate-900">{{ record.invoice_id }}</td><td class="px-4 py-3.5 font-medium text-slate-900">{{ record.student?.full_name || record.student_id }}</td><td class="px-4 py-3.5 font-semibold text-slate-900">{{ formatCurrency(record.amount_due) }}</td><td class="px-4 py-3.5">{{ formatCurrency(record.amount_paid) }}</td><td class="px-4 py-3.5">{{ formatCurrency(record.balance_due ?? Math.max(Number(record.amount_due || 0) - Number(record.amount_paid || 0), 0)) }}</td><td class="px-4 py-3.5"><Badge :type="record.payment_status === 'Paid' ? 'present' : 'warning'" :text="record.payment_status" /></td><td class="px-4 py-3.5"><div class="flex flex-wrap gap-2"><button type="button" @click="openPayment(record)" class="btn-primary px-2.5 py-1.5 text-xs font-semibold">Record payment</button><button type="button" @click="openEdit(record)" class="btn-secondary px-2.5 py-1.5 text-xs font-semibold">Edit</button><button type="button" :disabled="deletingId === record.invoice_id" @click="deleteRecord(record)" class="btn-danger px-2.5 py-1.5 text-xs font-semibold">{{ deletingId === record.invoice_id ? 'Deleting…' : 'Delete' }}</button></div></td></tr></tbody></table></div>
      </div>
    </div>
    <div v-if="editingId" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4" role="dialog" aria-modal="true" aria-labelledby="edit-financial-title"><form class="max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-4 shadow-xl sm:p-6" @submit.prevent="saveEdit"><div class="flex items-start justify-between"><div><h2 id="edit-financial-title" class="text-lg font-bold text-slate-900">Edit financial record</h2><p class="mt-1 text-xs text-slate-500">{{ editingRecord?.invoice_id }}</p></div><button type="button" @click="closeEdit" class="btn-ghost p-1 text-slate-400 hover:text-slate-700" aria-label="Close edit form">✕</button></div><div class="mt-5 grid gap-4 sm:grid-cols-2"><label class="text-xs font-semibold text-slate-700">Amount due (XAF)<input v-model="form.amount_due" type="number" min="1" step="1" inputmode="numeric" class="mt-1.5 block w-full rounded-md border border-slate-200 px-3 py-2 text-sm font-normal focus:border-blue-500 focus:outline-none" :aria-invalid="Boolean(formErrors.amount_due)" /></label><label class="text-xs font-semibold text-slate-700">Amount paid (XAF)<input v-model="form.amount_paid" type="number" min="0" step="1" inputmode="numeric" class="mt-1.5 block w-full rounded-md border border-slate-200 px-3 py-2 text-sm font-normal focus:border-blue-500 focus:outline-none" :aria-invalid="Boolean(formErrors.amount_paid)" /></label><label class="text-xs font-semibold text-slate-700">Payment status<select v-model="form.payment_status" class="mt-1.5 block w-full rounded-md border px-3 py-2 text-sm"><option v-for="status in paymentStatuses" :key="status" :value="status">{{ status }}</option></select></label><label class="text-xs font-semibold text-slate-700">Due date<input v-model="form.due_date" type="date" class="mt-1.5 block w-full rounded-md border px-3 py-2 text-sm" /></label></div><p v-if="formError" class="mt-4 rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700" role="alert">{{ formError }}</p><div class="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end sm:gap-3"><button type="button" @click="closeEdit" class="btn-secondary px-4 py-2 text-xs font-semibold">Cancel</button><button type="submit" :disabled="saving" class="btn-primary px-4 py-2 text-xs font-semibold text-white disabled:opacity-50">{{ saving ? 'Saving…' : 'Save changes' }}</button></div></form></div>
    <div v-if="paymentInvoice" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4" role="dialog" aria-modal="true" aria-labelledby="manual-payment-title"><form class="max-h-[calc(100dvh-2rem)] w-full max-w-2xl overflow-y-auto rounded-xl bg-white p-4 shadow-xl sm:p-6" @submit.prevent="saveManualPayment"><div class="flex items-start justify-between"><div><h2 id="manual-payment-title" class="text-lg font-bold text-slate-900">Record manual payment</h2><p class="mt-1 text-xs text-slate-500">{{ paymentInvoice.invoice_id }} · Balance {{ formatCurrency(paymentBalance) }}</p></div><button type="button" @click="closePayment" class="btn-ghost p-1 text-slate-400 hover:text-slate-700" aria-label="Close payment form">✕</button></div><div class="mt-5 grid gap-4 sm:grid-cols-2"><label class="text-xs font-semibold text-slate-700">Amount (XAF)<input v-model="paymentForm.amount" type="number" min="1" :max="paymentBalance" step="1" inputmode="numeric" required class="mt-1.5 block w-full rounded-md border px-3 py-2 text-sm" /></label><label class="text-xs font-semibold text-slate-700">Payment method<select v-model="paymentForm.payment_method" class="mt-1.5 block w-full rounded-md border bg-white px-3 py-2 text-sm"><option v-for="method in paymentMethods" :key="method" :value="method">{{ method }}</option></select></label><label class="text-xs font-semibold text-slate-700">Receipt number<input v-model.trim="paymentForm.receipt_number" required maxlength="80" placeholder="RCT-2026-0001" class="mt-1.5 block w-full rounded-md border px-3 py-2 text-sm" /></label><label class="text-xs font-semibold text-slate-700">Payment reference<input v-model.trim="paymentForm.payment_reference" maxlength="120" placeholder="Optional bank or manual reference" class="mt-1.5 block w-full rounded-md border px-3 py-2 text-sm" /></label><label class="text-xs font-semibold text-slate-700 sm:col-span-2">Paid at<input v-model="paymentForm.paid_at" type="datetime-local" required class="mt-1.5 block w-full rounded-md border px-3 py-2 text-sm" /></label><label class="text-xs font-semibold text-slate-700 sm:col-span-2">Notes<textarea v-model.trim="paymentForm.notes" maxlength="500" rows="2" class="mt-1.5 block w-full rounded-md border px-3 py-2 text-sm"></textarea></label></div><div v-if="paymentHistory.length" class="mt-5 rounded-lg bg-slate-50 p-3"><h3 class="text-xs font-bold uppercase tracking-wide text-slate-600">Payment history</h3><div v-for="payment in paymentHistory" :key="payment.payment_id" class="flex flex-col gap-1 border-b py-2 text-xs last:border-0 sm:flex-row sm:items-center sm:justify-between"><span>{{ payment.receipt_number }} · {{ payment.payment_method }}</span><strong>{{ formatCurrency(payment.amount) }}</strong></div></div><p v-if="paymentError" class="mt-4 rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700">{{ paymentError }}</p><div class="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end sm:gap-3"><button type="button" @click="closePayment" class="btn-secondary px-4 py-2 text-xs font-semibold">Cancel</button><button type="submit" :disabled="paymentSaving || paymentBalance <= 0" class="btn-primary px-4 py-2 text-xs font-semibold text-white disabled:opacity-50">{{ paymentSaving ? 'Saving…' : 'Save manual payment' }}</button></div></form></div>
  </div>
</template>
...
async function saveFeeSetting(setting) {
  const feeError = xafAmount(setting.fee_xaf, 'Fee', { positive: true })
  const credits = Number(setting.max_credits)
  if (feeError || !Number.isInteger(credits) || credits < 0) { errorMessage.value = feeError || 'Credits must be a non-negative whole number.'; return }
...
async function saveEdit() {
  formError.value = ''; formErrors.amount_due = xafAmount(form.amount_due, 'Amount due', { positive: true }); formErrors.amount_paid = xafAmount(form.amount_paid, 'Amount paid')
...
async function saveManualPayment() {
  paymentError.value = ''
  const amountError = xafAmount(paymentForm.amount, 'Payment amount', { positive: true })
  const amount = Number(paymentForm.amount)
  if (amountError || amount > paymentBalance.value) { paymentError.value = amountError || 'Enter an amount that does not exceed the outstanding whole-number XAF balance.'; return }
