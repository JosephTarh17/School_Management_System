        ...
        <div class="space-y-3 md:hidden">
          <article v-for="record in transactions" :key="record.invoice_id" class="rounded-lg border border-slate-200 bg-slate-50/60 p-4">
            ...
            <div class="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3">
              <button type="button" @click="openPayment(record)" class="rounded border border-emerald-200 px-2.5 py-2 font-semibold text-emerald-700 hover:bg-emerald-50">Record payment</button>
              <button type="button" @click="openEdit(record)" class="rounded border border-blue-200 px-2.5 py-2 font-semibold text-blue-700 hover:bg-blue-50">Edit</button>
              <button type="button" :disabled="deletingId === record.invoice_id" @click="deleteRecord(record)" class="rounded border border-rose-200 px-2.5 py-2 font-semibold text-rose-700 hover:bg-rose-50 disabled:opacity-50">{{ deletingId === record.invoice_id ? 'Deleting…' : 'Delete' }}</button>
            </div>
          </article>
        </div>
