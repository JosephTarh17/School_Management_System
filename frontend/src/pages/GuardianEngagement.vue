...
async function loadAll() {
  const results = await Promise.all([fetchGuardianCommunications(token()), fetchGuardianAppointments(token()), fetchGuardianDisciplineNotices(token()), fetchGuardianDocuments(token()), fetchGuardianProfile(token()), fetchGuardianProfileChangeRequests(token())])
...
onMounted(async () => {
  const childResult = await fetchGuardianChildren(token())
  if (childResult.ok) { children.value = childResult.data || []; appointmentForm.student_id = children.value[0]?.student_id || ''; }
  else errorMessage.value = childResult.error || 'Unable to load linked children.'
  await loadAll()
})
</script>
